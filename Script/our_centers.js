      ...
      centers.forEach((center, index) => {
        centerHTML += `
          <div>
            ...
            <p><strong>NPO Number:</strong> ${center.npoNumber}</p>
            <p><strong>Municipality:</strong> ${center.municipality}</p>
            <p><strong>Banking Details:</strong> ${center.bankingDetails}</p>
            <p><strong>Estimated number of people fed per day:</strong> ${center.peopleFed}</p>
            <button onclick="editCenter(${index})"><i class="fas fa-edit"></i></button>
            <button onclick="deleteCenter(${index})"><i class="fas fa-trash"></i></button>
          </div>
        `;
      });
